...
function toggleAdminMode() {
    ...
    window.isAdminMode = isAdminMode; // 전역 변수로 노출
    updateAdminModeUI();
    updateAdminModeVisibility();
    
    // 관리자 모드 상태를 localStorage에 저장
    localStorage.setItem('lrqa_admin_mode', isAdminMode);
    
    console.log('관리자 모드:', isAdminMode ? '활성화' : '비활성화');
}

// 관리자 모드 UI 업데이트
function updateAdminModeUI() {
    const adminButton = document.getElementById('adminModeToggle');
    if (adminButton) {
        if (isAdminMode) {
            adminButton.textContent = '관리자 모드 (ON)';
            adminButton.style.backgroundColor = '#28a745';
...
function updateAdminModeVisibility() {
    // LRQA 담당자용 정식 견적서 생성 버튼 표시/숨김
    const lrqaButtons = document.querySelectorAll('.btn-generate-lrqa-document');
    lrqaButtons.forEach(button => {
        if (isAdminMode) {
    ...
    // QuotationCalculator가 존재하면 관리자 모드 가시성 업데이트
    if (window.quotationCalculator && typeof window.quotationCalculator.updateAdminModeVisibility === 'function') {
        window.quotationCalculator.updateAdminModeVisibility();
    }
}

// 페이지 로드 시 관리자 모드 상태 복원
function restoreAdminMode() {
    const savedAdminMode = localStorage.getItem('lrqa_admin_mode');
