...
# -*- coding: utf-8 -*-
"""
Word 템플릿 파일 수정 도구 v2 - 조건부 태그 정확한 수정
"""
...
def fix_template_tags_v2(template_path, output_path):
    """템플릿 파일의 태그들을 정확하게 수정합니다."""
    try:
        # 원본 파일 백업
        backup_path = template_path.replace('.docx', '_backup_v2.docx')
        ...
        with zipfile.ZipFile(template_path, 'r') as zip_file:
            ...
            # 3. 조건부 태그 정확한 수정
            # {% if %} 태그와 {% endif %} 태그를 정확히 매칭
            if_tags = re.findall(r'\{%\s*if\s+[^%]+\s*%\}', xml_content)
            endif_tags = re.findall(r'\{%\s*endif\s*%\}', xml_content)
            
            print(f"   - if 태그 개수: {len(if_tags)}")
            print(f"   - endif 태그 개수: {len(endif_tags)}")
            
            # {% endif %} 태그가 {% if %} 태그보다 많으면 제거
            if len(endif_tags) > len(if_tags):
                # {% endif %} 태그를 if_tags 개수만큼만 유지
                count = 0
                def replace_endif(match):
                    nonlocal count
                    count += 1
                    return match.group(0) if count <= len(if_tags) else ''
            ...
            # 5. 잘못된 태그 이름들 수정
            # 분리된 태그들을 정리
            xml_content = re.sub(r'\{\{\s*iso9001_stage1_2_cost\s*</w:t></w:r>.*?format_currency\s*\}\}', 
                               '{{ iso9001_stage1_2_cost | format_currency }}', xml_content, flags=re.DOTALL)
            
            xml_content = re.sub(r'\{\{\s*iso14001_stage1_2_cost\s*</w:t></w:r>.*?format_currency\s*\}\}', 
                               '{{ iso14001_stage1_2_cost | format_currency }}', xml_content, flags=re.DOTALL)
            
            xml_content = re.sub(r'\{\{\s*iso45001_stage1_2_cost\s*</w:t></w:r>.*?format_currency\s*\}\}', 
                               '{{ iso45001_stage1_2_cost | format_currency }}', xml_content, flags=re.DOTALL)
            
            xml_content = re.sub(r'\{\{\s*total_cost_with_travel\s*</w:t></w:r>.*?format_currency\s*\}\}', 
                               '{{ total_cost_with_travel | format_currency }}', xml_content, flags=re.DOTALL)
            
            # 6. 기타 분리된 태그들 수정
            xml_content = re.sub(r'\{\{\s*iso9001_stage1_2_days\s*\}\}', '{{ iso9001_stage1_2_days }}', xml_content)
            xml_content = re.sub(r'\{\{\s*iso9001_surveillance_days\s*\}\}', '{{ iso9001_surveillance_days }}', xml_content)
            xml_content = re.sub(r'\{\{\s*iso14001_stage1_2_days\s*\}\}', '{{ iso14001_stage1_2_days }}', xml_content)
            xml_content = re.sub(r'\{\{\s*iso14001_surveillance_days\s*\}\}', '{{ iso14001_surveillance_days }}', xml_content)
            xml_content = re.sub(r'\{\{\s*iso45001_stage1_2_days\s*\}\}', '{{ iso45001_stage1_2_days }}', xml_content)
            xml_content = re.sub(r'\{\{\s*iso45001_surveillance_days\s*\}\}', '{{ iso45001_surveillance_days }}', xml_content)
            xml_content = re.sub(r'\{\{\s*total_audit_days\s*\}\}', '{{ total_audit_days }}', xml_content)
...
def main():
    ...
    fixed_template_path = 'vercel-deploy/public/templates/LRQA_quotation_fixed_v2.docx'
    
    if not os.path.exists(template_path):
        print(f"❌ 템플릿 파일을 찾을 수 없습니다: {template_path}")
        return
    
    print(f"🔧 템플릿 파일 수정 시작 v2: {template_path}")
    print("="*80)
    
    # 템플릿 수정
    if fix_template_tags_v2(template_path, fixed_template_path):
        print("\n✅ 템플릿 수정 완료!")
