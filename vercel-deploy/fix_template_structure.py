...
def fix_template_structure():
    """템플릿 구조를 수정하여 변수들을 올바른 위치로 이동합니다."""
            ...
            with zipfile.ZipFile(template_path + '.tmp', 'w', zipfile.ZIP_DEFLATED) as zip_write:
                for file_info in zip_read.infolist():
                    ...
                    if file_info.filename == "word/document.xml":
                        ...
                        print(f"\n=== {file_info.filename} 구조 수정 중 ===")
                        
                        # </w:document> 태그 밖에 있는 변수들을 찾아서 제거
                        # </w:document> 태그 이후의 모든 내용을 제거
                        ...
                        if re.search(document_end_pattern, content_str, re.DOTALL):
                            ...
                            print("✅ </w:document> 태그 밖의 변수들 제거 완료")
                        
                        # 이제 올바른 위치에 변수들을 추가
                        # </w:body> 태그 바로 앞에 변수들을 추가
                        body_end_pattern = r'(</w:body>)'
                        if re.search(body_end_pattern, content_str):
                            # 추가할 변수들
                            variables_to_add = [
                                '{{ quotation_date }}',
                                '{{ quotation_number }}',
                            ...
                            print(f"✅ {len(variables_to_add)}개 변수를 올바른 위치에 추가")
                        
                        # 변경사항이 있는지 확인
                        if content_str != original_content:
                            print(f"✅ 템플릿 구조 수정 완료")
                        else:
                            print(f"⚠️  변경사항 없음")
